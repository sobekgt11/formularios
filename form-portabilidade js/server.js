...
app.get('/form-portabilidade', async (req, res) => {
    const hash = req.query.hash;

    try {
        const response = await axios.get(`${process.env.API_PYTHON_URL}/get-form-data`, { params: { hash } });
        const formData = response.data;

        if (formData.status === "Concluido") {
            res.send('<h1 style="text-align: center; margin-top: 20%;">CONCLUIDO</h1>');
        } else {
            res.sendFile(path.join(__dirname, 'public', 'index.html'));
        }
    } catch (error) {
        console.error(error);
        res.status(404).send('<h1 style="text-align: center; margin-top: 20%;">Formulário não encontrado</h1>');
    }
});

app.post('/generate-form-link', async (req, res) => {
    try {
        const response = await axios.post(`${process.env.API_PYTHON_URL}/generate-form-link`, req.body);
        res.json(response.data);
    } catch (error) {
        console.error(error);
        res.status(500).send('Erro ao gerar link do formulário');
    }
});

app.post('/update-form-data', async (req, res) => {
    try {
        const response = await axios.post(`${process.env.API_PYTHON_URL}/update-form-data`, req.body);
        res.json(response.data);
    } catch (error) {
        console.error(error);
        res.status(500).send('Erro ao atualizar os dados do formulário');
    }
});
...
app.post('/generate-pdf', async (req, res) => {
    try {
        ...
        if (!formData || Object.keys(formData).length === 0) {
            throw new Error("Nenhum dado do formulário foi recebido");
        }
        ...
        const htmlContent = `
            ...
            <body>
                <div class="container">
                    ...
                    <span class="bold">${formData.novoDistribuidor || ''}</span>, inscrito no CNPJ/MF sob o nº <span class="bold">${formData.cnpjNovoDistribuidor || ''}</span>, mantendo a titularidade da(s) minha(s) posição(ões) de investimento.</p>
...
app.get('/config', (req, res) => {
    res.json({ apiPythonUrl: process.env.API_PYTHON_URL });
});
